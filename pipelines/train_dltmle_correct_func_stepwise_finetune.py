...
def _adapt_dataset(split_data, cf_seq, y_max, y_min, dataset_kind: str):
    ...
    raise ValueError(f"Invalid dataset kind: {dataset_kind}")
def _freeze_all_except_q_head(model: torch.nn.Module) -> None:
    """
    Freeze all parameters except the Q-head.

    For the DLTMLE variants used here, the Q-head parameters are `logit_Q*`
    (either a Linear or ModuleList).
    """
    for name, p in model.named_parameters():
        p.requires_grad = name.startswith("logit_Q")
...
def main(args: DictConfig):
    ...
    dataset_collections = {}
    cf_thresholds_seqs = [tuple(list(seq)) for seq in args.dataset.cf_seq_thresholds_stepwise]
    for cf_thresholds_seq in cf_thresholds_seqs:
        if args.dataset.name.startswith("mimic_extract_complex_func_stepwise"):
            ...
            dataset_collections[cf_thresholds_seq] = MIMICSynDatasetCollection(
                ...
                cf_treatment_sequence=[create_treat_func(threshold) for threshold in cf_thresholds_seq],
                seed=exp_seed,
            )
    ...
    for cf_thresholds_seq, dataset_collection in dataset_collections.items():
        logger.info(f"Dataset loaded for cf_seq_threshold: {cf_thresholds_seq}")
    ...
    # Step 1: Load ground truth CAPO for each stepwise threshold sequence
    ...
    if args.model.get("tune_hparams", False):
        cf_thresholds_seq = cf_thresholds_seqs[1]  # use the second stepwise CF policy for tuning
        cf_seq = [create_treat_func(threshold) for threshold in cf_thresholds_seq]
        logger.info(f"Tuning hyperparameters for cf_seq_threshold={cf_thresholds_seq} ...")
    ...
    logger.info("Training one base dltmle_correct model using the first stepwise counterfactual only...")

    base_cf_thresholds_seq = tuple(cf_thresholds_seqs[1])
    base_cf_seq = [create_treat_func(threshold) for threshold in base_cf_thresholds_seq]
    base_args = deepcopy(args)
    if best_hparams:
        for k, v in best_hparams.items():
            if hasattr(base_args.model, k):
                setattr(base_args.model, k, v)

    base_concat_dataset = _combine_train_val(
        dataset_collections[base_cf_thresholds_seq].train_f, dataset_collections[base_cf_thresholds_seq].val_f, args.dataset.name
    )
    base_concat_dataset_adapted = _adapt_dataset(
        base_concat_dataset, base_cf_seq, y_max, y_min, args.dataset.name
    )
    base_model, base_trainer = train_dltmle_correct_model_separate(
        base_concat_dataset_adapted, None, base_args
    )

    # Snapshot base weights on CPU for cheap cloning across sequences.
    base_state_dict = {k: v.detach().cpu() for k, v in base_model.state_dict().items()}

    logger.info(
        "Fine-tuning Q-head per counterfactual sequence (freezing all other parameters)..."
    )
    for i, cf_thresholds_seq in enumerate(cf_thresholds_seqs):
        seq_t = tuple(cf_thresholds_seq)
        cf_seq = [create_treat_func(threshold) for threshold in cf_thresholds_seq]
        if i == 0:
            model = base_model
            trainer = base_trainer
        else:
            # Per-sequence args (apply tuned hyperparameters)
            ...
            # combine train and val datasets for fine-tuning / final estimates
            concat_dataset = _combine_train_val(
                dataset_collections[seq_t].train_f, dataset_collections[seq_t].val_f, args.dataset.name
            )
            concat_dataset_adapted = _adapt_dataset(concat_dataset, cf_seq, y_max, y_min, args.dataset.name)

            # Clone base model weights and fine-tune only Q-head
            model_config = seq_args.model
            model = dltmle_correct(
                dim_static=seq_args.dataset.dim_static,
                dim_dynamic=seq_args.dataset.dim_dynamic,
                projection_horizon=seq_args.dataset.T - 1,
                dim_treatments=seq_args.dataset.dim_treatments,
            ...
            _freeze_all_except_q_head(model)
            logger.info(
                f"cf_seq_threshold={seq_t}: trainable params after freezing = {_count_trainable_params(model):,}"
            )

            # Optional overrides for fine-tuning (kept backwards-compatible with existing configs).
            finetune_max_epochs = int(seq_args.exp.get("finetune_max_epochs", seq_args.exp.max_epochs))
            finetune_lr = float(seq_args.model.get("finetune_learning_rate", seq_args.model.learning_rate))
            model.learning_rate = finetune_lr

            model, trainer = train_dltmle_correct_model_separate(
                concat_dataset_adapted, None, seq_args, model=model, max_epochs=finetune_max_epochs
            )

        # No sample splitting: evaluate on the same factual population used for final fitting.
        eval_split = base_concat_dataset if i == 0 else concat_dataset
        eval_dataset_adapted = _adapt_dataset(eval_split, cf_seq, y_max, y_min, args.dataset.name)
        capo_results = estimate_capo_for_one_sequence(
            model, trainer, eval_dataset_adapted, cf_thresholds_seq, ground_truth_apo, y_max=y_max, y_min=y_min
        )
    ...
    per_seq_results = {}
    for cf_thresholds_seq in cf_thresholds_seqs:
        seq_key = cf_thresholds_seq[0]
        per_seq_results[seq_key] = {
            "gcomp_estimate": results[seq_key]["gcomp_results"],
            "gcomp_bias": results[seq_key]["gcomp_bias"],
            "ltmle_estimate": results[seq_key]["ltmle_results"],
            "ltmle_bias": results[seq_key]["ltmle_bias"],
        }
...
def train_dltmle_correct_model_separate(train_dataset, val_dataset, args, model=None, max_epochs=None):
    """Train a dltmle_correct model (optionally using a provided model instance)."""
    ...
    _max_epochs = int(args.exp.max_epochs if max_epochs is None else max_epochs)
    trainer = Trainer(
        accelerator="gpu" if (len(args.exp.gpus) > 0) and (torch.cuda.is_available()) else 'cpu', 
...
def estimate_capo_for_one_sequence(model, trainer, eval_dataset, cf_thresholds_seq, ground_truth_apo, y_max: float, y_min: float):
    """Estimate CAPO for each stepwise counterfactual sequence."""
    seq_key = cf_thresholds_seq[0]
    logger.info(f"Estimating CAPO for cf_seq_threshold {seq_key}: {cf_thresholds_seq}")
    ...
    # Unscale estimates back to original Y units before comparing to ground truth.
    scale = (y_max - y_min)
    est_gcomp = float(est_gcomp) * scale + y_min
    gcomp_results = est_gcomp
    gcomp_bias = est_gcomp - ground_truth_apo[str(seq_key)]
    ...
    ltmle_bias = est_ltmle - ground_truth_apo[str(seq_key)]
    
    logger.info(f"Results for {seq_key}:")
    logger.info(f"  Ground truth: {ground_truth_apo[str(seq_key)]:.4f}")
...
if __name__ == "__main__":
    ...
    if not any(a.startswith("+model=") or a.startswith("model=") for a in argv_rest):
        sys.argv.append("+model=dltmle_correct_finetune_func_numz5")
    main()
