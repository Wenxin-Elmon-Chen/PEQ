...
def compute_ground_truth_capo_mimic_extract(args: DictConfig) -> dict:
    ...
    cf_seqs = [tuple(seq) for seq in args.dataset.cf_seqs]
    ground_truth_capo = {}
    data_seed = int(args.dataset.get("seed", args.exp.seed))

    for i, cf_seq in enumerate(cf_seqs):
        logger.info(f"Computing ground truth for cf_seq {i+1}/{len(cf_seqs)}: {cf_seq}")
        ...
        if args.dataset.name == "mimic_extract_complex":
            ...
            raise ValueError(
                f"Unsupported dataset: {args.dataset.name}. Expected mimic_extract or mimic_extract_complex."
            )
        ...
        gt = y_terminal.mean()
        ground_truth_capo[cf_seq] = gt

        logger.info(f"Ground truth CAPO for {cf_seq}: {gt:.6f}")
...
def main(args: DictConfig):
    """
    Compute and export ground truth CAPO for all counterfactual sequences for mimic_extract (non-stepwise cf_seqs).
    """
    OmegaConf.set_struct(args, False)
    logger.info("\n" + OmegaConf.to_yaml(args, resolve=True))

    name = str(getattr(args.dataset, "name", ""))
    if name not in ("mimic_extract", "mimic_extract_complex"):
        raise ValueError(
            f"Unsupported dataset: {name}. Use mimic_extract or mimic_extract_complex."
        )
    ...
    ground_truth_results = {
        ...
        "ground_truth_capo": {str(seq): float(val) for seq, val in ground_truth_capo.items()},
    }
