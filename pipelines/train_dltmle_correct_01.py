...
from pipelines.pipeline_utils import load_ground_truth_capo, export_capo_results_multi, infer_y_bounds_from_mimic_splits, _setup_file_logging

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)
torch.set_default_dtype(torch.double)

torch.set_num_threads(9)          # intra-op
...
def tune_hyperparameters(train_dataset, val_dataset, args):
    ...
    for trial in range(args.model.tune_range):
        ...
        try:
            ...
            # Create data loaders
            batch_size = int(trial_args.model.get("batch_size", 128))
...
def main(args: DictConfig):
    ...
    if args.model.get("tune_hparams", False):
        cf_seq = cf_seqs[0] # use the first cf_seq for tuning
        logger.info(f"Tuning hyperparameters for cf_seq={cf_seq} ...")
        train_dataset_adapted = _adapt_dataset(dataset_collections[cf_seq].train_f, cf_seq, y_max, y_min, args.dataset.name)
        val_dataset_adapted = _adapt_dataset(dataset_collections[cf_seq].val_f, cf_seq, y_max, y_min, args.dataset.name)
    ...
    for cf_seq in cf_seqs:
        ...
        concat_dataset_adapted = _adapt_dataset(concat_dataset, list(seq_t), y_max, y_min, args.dataset.name)
        ...
        eval_dataset_adapted = _adapt_dataset(concat_dataset, list(seq_t), y_max, y_min, args.dataset.name)
        capo_results = estimate_capo_for_one_sequence(
            model, trainer, eval_dataset_adapted, cf_seq, ground_truth_apo, y_max=y_max, y_min=y_min
        )
        results[seq_t] = capo_results
    ...
    for cf_seq in cf_seqs:
        ...
        per_seq_results[seq_t] = {
            "gcomp_estimate": results[seq_t]["gcomp_results"],
            "gcomp_bias": results[seq_t]["gcomp_bias"],
            "ltmle_estimate": results[seq_t]["ltmle_results"],
            "ltmle_bias": results[seq_t]["ltmle_bias"],
        }
...
def train_dltmle_correct_model_separate(train_dataset, val_dataset, args):
    ...
    val_dataloader = None if val_dataset is None else DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
...
def estimate_capo_for_one_sequence(model, trainer, eval_dataset, cf_seq, ground_truth_apo, y_max: float, y_min: float):
    ...
    gcomp_bias = est_gcomp - ground_truth_apo[tuple(cf_seq)]
    ...
    ltmle_bias = est_ltmle - ground_truth_apo[tuple(cf_seq)]
...
if __name__ == "__main__":
    ...
    if not any(a.startswith("+model=") or a.startswith("model=") for a in argv_rest):
        sys.argv.append("+model=dltmle_correct_01_tuned_numz5")
    main()
