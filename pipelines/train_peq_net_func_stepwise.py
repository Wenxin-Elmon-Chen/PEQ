...
def main(args: DictConfig):
    ...
    for cf_thresholds_seq, dataset_collection in dataset_collections.items():
        logger.info(f"Dataset loaded for cf_thresholds_seq: {cf_thresholds_seq}")
    ...
    for cf_thresholds_seq in args.dataset.cf_seq_thresholds_stepwise:
        ...
        per_seq_results[seq_name] = {
            "gcomp_estimate": capo_results["gcomp_results"][seq_name],
            "gcomp_bias": capo_results["gcomp_bias"][seq_name],
            "ltmle_estimate": capo_results["ltmle_results"][seq_name],
            "ltmle_bias": capo_results["ltmle_bias"][seq_name],
        }
...
def prepare_policy_embeddings(dataset_collections, args, policy_func, y_max: float, y_min: float):
    """Generate datasets and policy embeddings for all counterfactual sequences"""
    train_datasets = {}
    val_datasets = {}
    eval_datasets = {}
    policy_embeddings_by_time = {}

    # Stable indexing for per-policy diagnostics/logging.
    sorted_thresholds = sorted(list(dataset_collections.keys()), key=lambda seq: tuple(seq))
    policy_idx_by_id = {pid[0]: i for i, pid in enumerate(sorted_thresholds)}

    for cf_thresholds_seq, dataset_collection in dataset_collections.items():
        cf_seq = [policy_func(threshold) for threshold in cf_thresholds_seq]
        if args.dataset.name.startswith("mimic_extract"):
            train_dataset_adapted = MIMICDGPAdapter_CF(
                dataset_collection.train_f,
                cf_seq,
                y_max,
                y_min,
                policy_id=cf_thresholds_seq[0],
                policy_idx=policy_idx_by_id[cf_thresholds_seq[0]],
            )
        else:
            raise ValueError(f"Unsupported dataset (expected mimic_extract_func): {args.dataset.name}")
        train_features_by_time = create_policy_features(train_dataset_adapted, max_time_steps=args.dataset.T)
        policy_embeddings_by_time[cf_thresholds_seq[0]] = train_features_by_time
    
    gamma = 1 if not hasattr(args.model, 'gamma') else args.model.gamma
    mmd_results = compute_mds_coordinates_per_timestep_mmd(
        policy_embeddings_by_time,
        gamma=gamma,
        n_components=args.model.policy_embedding_dim,
    )
    ...
    for cf_thresholds_seq, dataset_collection in dataset_collections.items():
        cf_seq = [policy_func(threshold) for threshold in cf_thresholds_seq]
        policy_embeddings = []
        for t in range(len(cf_seq)):
            if mmd_results is not None:
                policy_embeddings_all = mmd_results['coordinates_by_time'][t]
                policy_ind = policy_embeddings_all['policy_ids'].index(cf_thresholds_seq[0])
                policy_embeddings.append(np.array(policy_embeddings_all['coordinates'][policy_ind]))
            else:
                policy_embeddings.append(np.array([cf_seq[t]]))
...
def estimate_capo_for_all_sequences(model, trainer, eval_datasets, cf_seq_thresholds_stepwise, ground_truth_apo, y_max: float, y_min: float):
    ...
    for i, cf_thresholds_seq in enumerate(cf_seq_thresholds_stepwise):
        seq_name = cf_thresholds_seq[0]
        logger.info(f"Estimating CAPO for cf_seq_threshold {i+1}/{len(cf_seq_thresholds_stepwise)}: {cf_thresholds_seq}")
        ...
        gcomp_results[seq_name] = est_gcomp
        gcomp_bias[seq_name] = est_gcomp - ground_truth_apo[str(seq_name)]
        ...
        ltmle_results[seq_name] = est_ltmle
        ltmle_bias[seq_name] = est_ltmle - ground_truth_apo[str(seq_name)]
        
        logger.info(f"Results for {seq_name}:")
        logger.info(f"  Ground truth: {ground_truth_apo[str(seq_name)]:.4f}")
        logger.info(f"  G-comp: {est_gcomp:.4f} (bias: {gcomp_bias[seq_name]:.4f})")
        logger.info(f"  LTMLE: {est_ltmle:.4f} (bias: {ltmle_bias[seq_name]:.4f})")
...
if __name__ == "__main__":
    ...
    if not any(a.startswith("+model=") or a.startswith("model=") for a in argv_rest):
        sys.argv.append("+model=peq_net_func_tuned_numz5")
    main()
