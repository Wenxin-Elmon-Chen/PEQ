...
from src.models import deepace_general_models as general
from src.models import deepace_helpers as helpers
...
def obj_deepace(a_hat, a_data, q_hat_f, q_hat_eps, q_hat_cf, y_data, alpha, beta):
    ...
    outcome_loss_last = torch.mean(outcome_loss_M[:,-1])
    return outcome_loss + alpha * treat_loss + beta * target_loss, outcome_loss, treat_loss, target_loss, outcome_loss_last


# Encoder------------------------------------------------------------
# Training procedure for encoder
def train_deepace(config, train_loader, val_loader, a_int, epochs=100, alpha=1, beta=1, tune_mode=False):
    # Data
    # Model
    deepace = DeepACE(config=config, input_size=train_loader.dataset[0].shape[1] - 1, a_int=a_int, alpha=alpha, beta=beta)
    deepace.set_tune_mode(tune_mode)
    # Train
    if not tune_mode:
        #neptune_logger = NeptuneLogger(project='dennisfrauen/SeqDragonNet')
        Trainer1 = pl.Trainer(max_epochs=epochs, logger=False, checkpoint_callback=False)# logger=neptune_logger)
    else:
        Trainer1 = pl.Trainer(max_epochs=epochs, logger=False, checkpoint_callback=False)
    ...
    loss_GQ = val_results[0]['val_loss_y'] + val_results[0]['val_loss_a']
    return deepace, val_err, loss_GQ
...
class DeepACE(general.Causal_Encoder):
    ...
    def format_input(self, data_torch):
        ...
        a_data = data_torch[:, :, 1]
        x_data = data_torch[:, :, 2:]
        a_input = torch.empty((data_torch.size(0), T, 1)).to(data_torch.device)
    ...
    def forward(self, input_fwd):
        ...
        batch_size = x.size(0)
        a_data = input_fwd[2]
        a_int_torch = torch.from_numpy(self.a_int)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        loss, y_loss, a_loss, loss_target, y_loss_last = obj_deepace(a_hat, a_data, q_hat_f, q_hat_eps, q_hat_cf, y_data,
                                                        self.alpha, self.beta * lamb)
        if not self.tune_mode:
            # Logging
            self.log('train_loss', loss.detach().numpy().item(), logger=(self.logger is not None), on_epoch=True, on_step=False)
            self.log('train_loss_a', a_loss.detach().numpy().item(), logger=(self.logger is not None), on_epoch=True, on_step=False)
            self.log('train_loss_y', y_loss.detach().numpy().item(), logger=(self.logger is not None), on_epoch=True, on_step=False)
            self.log('train_loss_target', loss_target.detach().numpy().item(), logger=(self.logger is not None), on_epoch=True, on_step=False)
            self.log('train_loss_last', y_loss_last.detach().numpy().item(), logger=(self.logger is not None), on_epoch=True, on_step=False)
        return loss
    ...
    def validation_step(self, train_batch, batch_idx):
        ...
        if self.beta > 0:
            ...
            lamb = torch.tensor(lamb, requires_grad=False)
        if not self.tune_mode:
            loss, loss_y, loss_a, loss_target, y_loss_last = obj_deepace(
                a_hat, a_data, q_hat_f, q_hat_eps, q_hat_cf, y_data, self.alpha, self.beta * lamb
            )
            ...
            self.log('val_loss_last', y_loss_last.detach().numpy().item(), logger=True, on_epoch=True, on_step=False)
        else:
            loss, loss_y, loss_a, loss_target, y_loss_last = obj_deepace(a_hat, a_data, q_hat_f, q_hat_eps, q_hat_cf, y_data, 0, 0)
            self.log('val_loss', loss.detach().numpy().item(), logger=True, on_epoch=True, on_step=False)

        return {'val_loss': loss, 'val_loss_y': loss_y, 'val_loss_a': loss_a, 'val_loss_last': y_loss_last}
    ...
    def estimate_avg_outcome(self, d_train_seq, y_scaler=None, dropout=False):
        if dropout==False:
            ...
            self.train()
        d_train_torch = torch.from_numpy(d_train_seq)
        # Forward pass
