...
class DLTMLE_correct_multiQhead(LightningModule):
    ...
    def __init__(self,
        ...
        self.logit_Q = nn.ModuleList([nn.Linear(d_model, 1) for _ in range(num_q_heads)])
    ...
    def _apply_policy_q_heads(self, z_q, policy_idx):
        """
        Apply the per-policy Q head for each sample in the batch.
        z_q: (batch_size, tau, d_model)
        policy_idx: (batch_size,) or (batch_size, 1)
        """
        policy_idx = policy_idx.view(-1).to(dtype=torch.long, device=z_q.device)
        if policy_idx.numel() != z_q.shape[0]:
            raise ValueError(
                f"policy_idx batch size ({policy_idx.numel()}) does not match z_q batch size ({z_q.shape[0]})."
            )

        if bool((policy_idx < 0).any()) or bool((policy_idx >= self.num_q_heads).any()):
            min_idx = int(policy_idx.min().item())
            max_idx = int(policy_idx.max().item())
            raise ValueError(
                f"policy_idx out of range: [{min_idx}, {max_idx}], expected in [0, {self.num_q_heads - 1}]"
            )

        # Vectorized per-sample head selection:
        # gather each sample's head params, then do one batched linear projection.
        all_weights = torch.cat([head.weight for head in self.logit_Q], dim=0)  # (K, d_model)
        all_bias = torch.cat([head.bias for head in self.logit_Q], dim=0)  # (K,)
        sample_weights = all_weights[policy_idx]  # (batch_size, d_model)
        sample_bias = all_bias[policy_idx]  # (batch_size,)

        out = torch.einsum("btd,bd->bt", z_q, sample_weights) + sample_bias[:, None]
        return out.unsqueeze(-1)

    def forward(self, W, L, A, Y, a, policy_idx, **kwargs):
        """
        ...
        policy_idx: `integer`
            index of the policy of the sample
        """
        ...
        # Use the policy-specific head for each sample.
        logit_Q_l[:, 1:] = self._apply_policy_q_heads(z_Q, policy_idx)
        ...
        logit_Q_a[:, :-1] = self._apply_policy_q_heads(z_A_t, policy_idx).detach()  # block back propagation
