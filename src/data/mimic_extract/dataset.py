...
from src.data.mimic_extract.simulation import simulate_treat_outcomes, simulate_treatment_outcomes_function
...
class MIMICSynDatasetCollection(SyntheticDatasetCollection_Simplified):
    ...
    def __init__(self, num_patients, config, h5file_path, cf_treatment_sequence, seed=42, **kwargs):
        ...
        # Use cached data loading for efficiency
        ...
        factual_train, _ = simulate_treat_outcomes({**config, 'n': num_patients['train']}, np.zeros((T)), train_vitals, seed=seed) # NOTE: the np.zeros((T)) is the counterfactual treatment sequence, but the counterfactual treatment sequence is output to _, so we don't need to worry about it
        ...
        factual_val, _ = simulate_treat_outcomes({**config, 'n': num_patients['val']}, np.zeros((T)), val_vitals, seed=seed + 1000) # NOTE: the np.zeros((T)) is the counterfactual treatment sequence, but the counterfactual treatment sequence is output to _, so we don't need to worry about it
    ...
    def _load_mimic_covariates_from_df(self, df, n, T, vital_list, static_list, seed, exclude_subject_ids=None):
        ...
        """
        from src.data.mimic_extract.simulation import standardize_covariates
        ...
        if n > len(filtered_users):
            raise ValueError(
                f"Not enough eligible subjects to sample n={n} after exclusion "
                f"(eligible={len(filtered_users)})."
            )
        ...
        if static_list is not None:
            static_features = static_features.loc[chosen_subject_ids]
            processed_static_features = []
            for feature in static_features.columns:
                if not isinstance(static_features[feature].iloc[0], float):
                    # Categorical feature - convert to numeric
                    unique_vals = static_features[feature].unique()
                    feature_map = {val: i for i, val in enumerate(unique_vals)}
                    processed_static_features.append([feature_map[val] for val in static_features[feature]])
                else:
                    # Numeric feature
                    processed_static_features.append(static_features[feature].tolist())
            
            processed_static_features = np.array(processed_static_features).T
